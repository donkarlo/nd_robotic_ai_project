...
from nd_robotic_ai.robot.composition.kind.mind.meta_cognition.cognition.process.composition.child.memory.composition.trace.interface import Interface as TraceInterface
from nd_robotic_ai.robot.composition.kind.mind.meta_cognition.cognition.process.composition.child.memory.composition.trace.kind.core.kind import Kind as TraceKind


class Trace(TraceInterface):
    """
    - This class is bridge between formatted data in form of a Dic and a society of ActionPotentials, so if you give it one, it should be abale to return back the other
        - it can be used as training pair between action potentioal society and formatted data
    ...
    - trace is the connection between body's nervous_system system and mind. on one hand it receives potential activity fieelds and on theother hand it converts it to a nested action_potential_group point such as a dictionary or a parts pattern
    - This is a lost in time_based memory cellgaller
    An role or a sensory read happens
    ...
    - action_potentials can not be decorated or be parts or decorated
    - trace covers from action_potential activit field to structed action_potential_group
    - this is the inner_experiment of parts pattern
    """
    ...
    def __init__(self, formatted_data:Optional[Any], action_potential_group: Optional[ActionPotentialGroup] , name: Optional[str]):
        ...
        # is it a action? a lidar sensory? a initial_mission? This must be guessed from formatted action_potential_group
