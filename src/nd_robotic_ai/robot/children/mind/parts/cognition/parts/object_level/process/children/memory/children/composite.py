...
class Composite(MemoryComponent, BaseComposite):
    ...
    def create_directory_structure(self, root_path:Path, leaf_file_extension: str) -> None:
        """
        For each RotorComposite: create a directory named after the parts under `root_path`.
        ...
        # MapToDirectory for this parts under root_path
        ...
        # Create files for direct leaves and recurse into parts parts
        for child in self.get_child_group_members():
            if child.is_leaf():
                ...
                # Recurse into parts parts
                # We assume parts is a RotorComposite that implements create_directory_structure.
