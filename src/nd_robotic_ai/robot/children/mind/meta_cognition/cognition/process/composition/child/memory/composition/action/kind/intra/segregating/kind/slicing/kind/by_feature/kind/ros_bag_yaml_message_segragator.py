...
from nd_robotic_ai.robot.structure.kind.mind.process.kind.memory.action.kind.intra.segregation.segregator.kind.slicer.feature.feature_slicer import \
    FeatureSlicer
...
from nd_robotic_ai.robot.composition.kind.mind.meta_cognition.cognition.process.composition.child.memory.composition.composite import Composite as MemoryComposite
...
from nd_robotic_ai.robot.composition.kind.mind.meta_cognition.cognition.process.composition.child.memory.composition.trace.group.decorator.storaged import Storaged
...
from nd_robotic_ai.robot.composition.kind.mind.meta_cognition.cognition.process.composition.child.memory.composition.trace.kind.core.kind import Kind as TraceGroupKind
from nd_robotic_ai.robot.composition.kind.mind.meta_cognition.cognition.process.composition.child.memory.composition.trace.group.kind.core.kind import Kind as TraceGroupKinds
...
class RosBagYamlMessageSegragating(FeatureSlicer, TraceAddValueSubscriber, GroupRamValuesAdditionFinishedSubscriber):
    ...
    def do_when_a_new_value_is_added_to_ram(self, value: Any) -> None:
        ...
        if isinstance(value, Dic):
            ...
            # note that trace children title is the same as trace action_potential_group children title in this case
            ...
            for trace_group_kind_name, trace_group in self._trace_groups_dic.get_keys_values():
                # we use starts with here because trace action_potential_group children title is has a slice string in the end
