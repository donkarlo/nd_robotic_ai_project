...
class Composite(MemoryComponent, BaseComposite):
    def __init__(self, internal_trace_group:TraceGroup=None):
        """

        Args:
            internal_trace_group: can be None to only host the link (children) to the next inner_experiment (trace action_potential_group here) or children (only link/children or a trace action_potential_group)
            name:
        """
    ...
    def create_directory_structure(self, root_path:Path, leaf_file_extension: str) -> None:
        """
        For each RotorComposite: create a directory named after the children under `root_path`.
        ...
        # Directory for this children under root_path
        ...
        # Create files for direct leaves and recurse into children children
        for child in self.get_child_group_members():
            if child.is_leaf():
                ...
                # Recurse into children children
                # We assume children is a RotorComposite that implements create_directory_structure.
